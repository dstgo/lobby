...
import (
	...
	"github.com/dstgo/lobby/server/data/ent/job"
...
func (jq *JobQuery) QueryOwner() *ContainerQuery {
	query := (&ContainerClient{config: jq.config}).Query()
...
func (jq *JobQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Job, error) {
	...
	if query := jq.withOwner; query != nil {
		if err := jq.loadOwner(ctx, query, nodes,
			func(n *Job) { n.Edges.Owner = []*Container{} },
			func(n *Job, e *Container) { n.Edges.Owner = append(n.Edges.Owner, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (jq *JobQuery) loadOwner(ctx context.Context, query *ContainerQuery, nodes []*Job, init func(*Job), assign func(*Job, *Container)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(job.OwnerTable)
		s.Join(joinT).On(s.C(container.FieldID), joinT.C(job.OwnerPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(job.OwnerPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(job.OwnerPrimaryKey[1]))
