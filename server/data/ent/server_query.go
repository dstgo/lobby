...
func (sq *ServerQuery) QueryTags() *TagQuery {
...
// QuerySecondaries chains the current query on the "secondaries" edge.
func (sq *ServerQuery) QuerySecondaries() *SecondaryQuery {
	query := (&SecondaryClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *ServerQuery) WithTags(opts ...func(*TagQuery)) *ServerQuery {
	query := (&TagClient{config: sq.config}).Query()
...
// WithSecondaries tells the query-builder to eager-load the nodes that are connected to
// the "secondaries" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *ServerQuery) WithSecondaries(opts ...func(*SecondaryQuery)) *ServerQuery {
	query := (&SecondaryClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withSecondaries = query
	return sq
}
...
func (sq *ServerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Server, error) {
	...
	if query := sq.withTags; query != nil {
		if err := sq.loadTags(ctx, query, nodes,
			func(n *Server) { n.Edges.Tags = []*Tag{} },
			func(n *Server, e *Tag) { n.Edges.Tags = append(n.Edges.Tags, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withSecondaries; query != nil {
		if err := sq.loadSecondaries(ctx, query, nodes,
			func(n *Server) { n.Edges.Secondaries = []*Secondary{} },
			func(n *Server, e *Secondary) { n.Edges.Secondaries = append(n.Edges.Secondaries, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ServerQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Server, init func(*Server), assign func(*Server, *Tag)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tag.FieldOwnerID)
	}
	query.Where(predicate.Tag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(server.TagsColumn), fks...))
	}))
...
func (sq *ServerQuery) loadSecondaries(ctx context.Context, query *SecondaryQuery, nodes []*Server, init func(*Server), assign func(*Server, *Secondary)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Server)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(secondary.FieldOwnerID)
	}
	query.Where(predicate.Secondary(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(server.SecondariesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
