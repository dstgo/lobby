...
func (tq *TagQuery) QueryServers() *ServerQuery {
	query := (&ServerClient{config: tq.config}).Query()
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withServers: tq.withServers.Clone(),
...
func (tq *TagQuery) WithServers(opts ...func(*ServerQuery)) *TagQuery {
	query := (&ServerClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withServers = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withServers; query != nil {
		if err := tq.loadServers(ctx, query, nodes, nil,
			func(n *Tag, e *Server) { n.Edges.Servers = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadServers(ctx context.Context, query *ServerQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Server)) error {
...
func (tq *TagQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withServers != nil {
			_spec.Node.AddColumnOnce(tag.FieldOwnerID)
		}
	}
