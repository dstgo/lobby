...
func (uq *UserQuery) QueryContainers() *ContainerQuery {
	query := (&ContainerClient{config: uq.config}).Query()
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withContainers: uq.withContainers.Clone(),
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withContainers; query != nil {
		if err := uq.loadContainers(ctx, query, nodes,
...
func (uq *UserQuery) loadContainers(ctx context.Context, query *ContainerQuery, nodes []*User, init func(*User), assign func(*User, *Container)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.ContainersTable)
		s.Join(joinT).On(s.C(container.FieldID), joinT.C(user.ContainersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.ContainersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.ContainersPrimaryKey[0]))
